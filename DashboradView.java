package com.huaweidun.imotionandroid.view;
...
import com.huaweidun.imotionandroid.R;
import com.huaweidun.imotionandroid.util.SpringInterpolator;
...
public class DashboradView extends View {
    ...
    private float oldPercent = mMin; // 实时读数
    private boolean isShowValue = true; // 是否显示实时读数
    private int mStrokeWidth; // 画笔宽度 private int mLength1; // 长刻度的相对圆弧的长度
    private int mStrokeWidthInner; // 画笔宽度 private int mLength1; //
    private int mStrokeWidReal;
    ...
    private Path mPath;
    private Rect mRectText;//实时角度显示
    private RectF mRectFArcInner; //实心
    private RectF mRectFArcQualified; //合格
    private RectF mRectFArcRealTime; //实时弧度
    private RectF mRectFInnerArc;   //写字用的
    private int mStartAngleQualified = 180; // 起始角度
    private int mSweepAngleQualified = 180; // 绘制角度
    ...
    private void init() {
        ...
        mStrokeWidthInner = dp2px(20);
        mStrokeWidReal = dp2px(6);
        mLength1 = dp2px(15) + mStrokeWidth;
        ...
        mRectFArcQualified = new RectF();
        mRectFArcRealTime = new RectF();
        mRectFInnerArc = new RectF();
        mPath = new Path();
        mRectText = new Rect();
        mTexts = new String[]{"起始", "合格", "警告"}; //
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = resolveSize(dp2px(270), widthMeasureSpec);
        ...
        mRectFArcInner.set(
                getPaddingLeft() + mStrokeWidthInner / 2 + mStrokeWidth,
                getPaddingTop() + mStrokeWidthInner / 2 + mStrokeWidth,
                getMeasuredWidth() - getPaddingRight() - mStrokeWidthInner / 2 + mStrokeWidth,
                getMeasuredWidth() - getPaddingBottom() - mStrokeWidthInner / 2 + mStrokeWidth
        );
        mRectFArcQualified.set(
                getPaddingLeft() + mStrokeWidthInner / 2 + mStrokeWidth,
                getPaddingTop() + mStrokeWidthInner / 2 + mStrokeWidth,
                getMeasuredWidth() - getPaddingRight() - mStrokeWidthInner / 2 + mStrokeWidth,
                getMeasuredWidth() - getPaddingBottom() - mStrokeWidthInner / 2 + mStrokeWidth
        );
        mRectFArcRealTime.set(
                getPaddingLeft() + mStrokeWidReal / 2 + mStrokeWidth,
                getPaddingTop() + mStrokeWidReal / 2,
                getMeasuredWidth() - getPaddingRight() - mStrokeWidReal / 2,
                getMeasuredWidth() - getPaddingBottom() - mStrokeWidReal / 2
        );
        mRectFInnerArc.set(
                getPaddingLeft() + mLength1,
                getPaddingTop() + mLength1,
                getMeasuredWidth() - getPaddingRight() - mLength1,
                getMeasuredWidth() - getPaddingBottom() - mLength1
        );
        mPLRadius = mRadius;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.yibiaopan_E5F0FF));
        ...
        mPaint.setStrokeWidth(mStrokeWidthInner);
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.yibiaopan_E5F0FF));
        ...
        mPaint.setStrokeCap(Paint.Cap.BUTT);
        mPaint.setStrokeWidth(mStrokeWidthInner);
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.yibiaopan_ADCEFC));
        canvas.drawArc(mRectFArcQualified, mStartAngleQualified, mSweepAngleQualified, false, mPaint);
        ...
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.color_dcdcdc));
        ...
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setStrokeCap(Paint.Cap.BUTT);
        mPaint.setStrokeWidth(mStrokeWidReal);
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.colorPrimary));
        canvas.drawArc(mRectFArcRealTime, mStartAngle, mRealTimeValue, false, mPaint);
        /**
         * 画长刻度读数
         * 添加一个圆弧path，文字沿着path绘制
         */
        mPaint.setTextSize(sp2px(8));
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.color_027FFF));
        mPaint.setTextAlign(Paint.Align.CENTER);
        mPaint.setStyle(Paint.Style.FILL);
        for (int i = 0; i < mTexts.length; i++) {
            mPath.reset();
            if (mTexts[i].equals("起始")) {
                mPath.addArc(
                        mRectFInnerArc,
                        mStartAngle,
                        mStartAngleQualified - mStartAngle
                );
            } else if (mTexts[i].equals("合格")) {
                mPath.addArc(
                        mRectFInnerArc,
                        mStartAngleQualified, // 正起始角度减去θ使文字居中对准长刻度
                        mSweepAngleQualified
                );
            } else {
                mPath.addArc(
                        mRectFInnerArc,
                        mStartAngleQualified + mSweepAngleQualified, // 正起始角度减去θ使文字居中对准长刻度
                        mSweepAngle * 2 - mStartAngleQualified - mSweepAngleQualified
                );
            }
            canvas.drawTextOnPath(mTexts[i], mPath, 0, dp2px(18), mPaint);
        }
        /**
         * 画指针
         */
        ...
        if (isShowValue) {
            ...
            String value = fnum.format(mRealTimeValue);
            mPaint.getTextBounds(value, 0, value.length(), mRectText);

            canvas.drawText("角度  " + value + "°", mCenterX, mCenterY + mPSRadius + mRectText.height() * 2, mPaint);
        }
    ...
    public void setRealTimeValue(float realTimeValue) {
       ...
       // oldPercent=mRealTimeValue;
        mRealTimeValue = realTimeValue;
       // setAnimator(realTimeValue);
        postInvalidate();
    }
    ...
    private void setAnimator(final float percent) {
        ...
        animatorDuration = (long) Math.abs(percent - oldPercent) * 25;
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                DashboradView.this.mRealTimeValue =  (float)animation.getAnimatedValue();
                postInvalidate();
        ...
        valueAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //防止因为插值器误差产生越界
                if (DashboradView.this.mRealTimeValue < 0.0) {
                    DashboradView.this.mRealTimeValue = 0;
                    postInvalidate();
                }
                if (DashboradView.this.mRealTimeValue > mMax) {
                    DashboradView.this.mRealTimeValue = mMin;
                    postInvalidate();
                }
            }
        });
    ...
    public int getmStartAngleQualified() {
        return mStartAngleQualified;
    }

    public void setmStartAngleQualified(int mStartAngleQualified) {
        this.mStartAngleQualified = mStartAngleQualified;
        postInvalidate();
    }

    public void setmSweepAngleQualified(int mSweepAngleQualified) {
        this.mSweepAngleQualified = mSweepAngleQualified;
        postInvalidate();
    }
}
